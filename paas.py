...
'''
            ...
            subprocess.run(['docker', 'run', '-d', '--name', f"{app_name}_container_{version}", '-p', f"{host_port}:80", registry_image_name], check=True)
...
def run_streamlit_ui():
    ...
    for app, version in deploy_and_save.version_map.items():
        ...
        update_button = col3.button(f"Update {app}")
        if update_button:
            asyncio.run(deploy_and_save.deploy(app, deploy_and_save.repo_map.get(app, '')))
        ...
        if delete_button:
            # Delete selected version or all
            if selected_version == 'all':
                # Delete all versions (use Docker commands to remove images)
                pass
            else:
                # Delete the selected version (use Docker commands to remove the specific image)
                pass
